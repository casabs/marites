                ...
                <div class="col-sm-9" id="user-section">
                    ...
                    <?php
                        ...
                        else
                        {
                            ...
                            $date1 = new DateTime();
                            $date2 = new DateTime($row['event_date'], new DateTimeZone('UTC'));
                            
                            $diff = $date1->diff($date2);
                            $diff_sec = $diff->s + ($diff->i * 60) + ($diff->h * 60 * 60) + ($diff->d * 24 * 60 * 60) + ($diff->m * 30 * 24 * 60 * 60) + ($diff->y * 365 * 24 * 60 * 60);
                            
                            $diff_sec_with_sign = $diff->format('%r') . abs($diff_sec);
                            
                        }
                    ?>

                    <img class="event_cover" src="uploads/<?php echo $row['event_image']; ?>"><br>
