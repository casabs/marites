...
class poll {
    ...
    public function getPollOption($pollOptionId){
        $sql = "SELECT * FROM ".$this->optTbl." WHERE id = ".$pollOptionId." AND status = '1'";
        $optionResult = $this->getQuery($sql, 'single');
        return !empty($optionResult)?$optionResult:false;
    }
